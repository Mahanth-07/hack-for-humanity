...
async function seedNeon() {
  ...
  try {
    await db.execute(sql`DELETE FROM camera_detections`);
    await db.execute(sql`DELETE FROM robocalls`);
    await db.execute(sql`DELETE FROM risk_assessments`);
    await db.execute(sql`DELETE FROM incidents`);
    await db.execute(sql`DELETE FROM contacts`);
    await db.execute(sql`DELETE FROM camera_feeds`);
    await db.execute(sql`DELETE FROM module_health`);
    console.log("Cleared existing data");
  } catch (e: any) {
    console.log("Tables may not exist yet, continuing...", e.message?.substring(0, 80));
  }
  ...
  const incidentsData = [
    { title: "Structure Fire - Warehouse District", description: "Multi-alarm fire reported at industrial warehouse. Heavy smoke visible.", severity: "critical", status: "active", location: "Warehouse District, Los Angeles, CA", coordinates: { lat: 34.0407, lng: -118.2468 } },
    { title: "Multi-Vehicle Collision - I-10", description: "5-car pileup on I-10 westbound. Multiple injuries reported.", severity: "high", status: "active", location: "I-10 Westbound, Houston, TX", coordinates: { lat: 29.749, lng: -95.358 } },
    { title: "Suspicious Package - Times Square", description: "Unattended package reported near Times Square subway entrance.", severity: "high", status: "active", location: "Times Square Station, New York, NY", coordinates: { lat: 40.756, lng: -73.987 } },
    { title: "Flooding - Lower Wacker Drive", description: "Flash flooding reported on Lower Wacker Drive. Road impassable.", severity: "medium", status: "active", location: "Lower Wacker Drive, Chicago, IL", coordinates: { lat: 41.887, lng: -87.627 } },
    { title: "Power Outage - South Beach", description: "Major power outage affecting 10 blocks of South Beach.", severity: "medium", status: "active", location: "South Beach, Miami, FL", coordinates: { lat: 25.783, lng: -80.131 } },
    { title: "Gas Leak - Capitol Hill", description: "Natural gas leak detected near residential area. Evacuation in progress.", severity: "high", status: "active", location: "Capitol Hill, Seattle, WA", coordinates: { lat: 47.625, lng: -122.322 } },
    { title: "Missing Person - Centennial Park", description: "Missing child reported near Centennial Olympic Park.", severity: "medium", status: "active", location: "Centennial Park, Atlanta, GA", coordinates: { lat: 33.7604, lng: -84.3931 } },
    { title: "Chemical Spill - Port Area", description: "Chemical spill at industrial port. Hazmat team dispatched.", severity: "critical", status: "active", location: "Port of New Orleans, LA", coordinates: { lat: 29.935, lng: -90.063 } },
    { title: "Traffic Signal Failure", description: "Multiple traffic signals malfunctioning at major intersection.", severity: "low", status: "active", location: "Peachtree & 14th, Atlanta, GA", coordinates: { lat: 33.786, lng: -84.383 } },
    { title: "Medical Emergency - Convention Center", description: "Mass casualty incident reported at convention center.", severity: "critical", status: "active", location: "Las Vegas Convention Center, NV", coordinates: { lat: 36.128, lng: -115.153 } },
  ];
  ...
  await db.insert(moduleHealth).values(modulesData);
  console.log(`Seeded ${modulesData.length} module health records`);
